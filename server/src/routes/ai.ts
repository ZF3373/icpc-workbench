...
export function aiRoutes(
  ...
  // ---------- PDF 文本提取（本地，不依赖 AI 配置） ----------
  // 客户端上传 PDF 时先调此端点提取文本，再以 textContent 注入对话。
  // 接收原始字节流 + content-type/x-file-name 头；仅处理 PDF，非 PDF 返回 400。
  ...
  r.post(
    ...
    raw({ type: () => true, limit: MAX_PDF_BYTES }),
    asyncHandler(async (req, res) => {
      const buf = req.body;
      if (!Buffer.isBuffer(buf) || buf.length === 0) {
        return res.status(400).json({ error: '请求体需为原始文件字节流（不可为空）' });
      ...
      // 仅处理 PDF：按 content-type 或文件名扩展名判断
      if (!isPdfContentType(contentType) && !isPdfFilename(filename)) {
        return res.status(400).json({ error: '此端点仅处理 PDF 文件，请确认文件类型' });
      }
      try {
